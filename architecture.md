...
| **Market prices** (BDT/kg by market/district) | **WFP via HDX** | 🟡 **Periodic** — monthly bulk CSV, refreshed on demand | CKAN `package_show` → `wfp_food_prices_bgd.csv` → `PriceObservation` | KB `/api/kb/prices` → revenue, break-even, profit | **REAL, populated** (359 rows, Oct 2024–May 2026) — labelled "WFP monthly", not a spot price |
