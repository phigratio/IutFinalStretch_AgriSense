...
- [x] **A1. Schema patch** — Prisma migration `add_agent_tier0_fields`: add `areaHa`, `district`,
...
- [x] **A2. Trace decorator** — `src/tools/trace.ts` `withTrace()` → writes one `AgentToolCall`
  row per tool call (name, params, rawResponse, status, timings). `[Must]`
- [x] **A3. Data files** — `src/data/*.csv` (crops, crop_calendar, crop_water, fertilizer_frg,
...
- [x] **A4. Loader + mock isolation** — one loader; Tier 0 reads reject `dataOrigin=mock`
  (spec §7.3). `[Must]`
- [x] **A5. Canonical crop IDs + alias table** — `rice_t_aman, rice_boro, wheat, maize, potato,
  mustard, lentil, onion`; Bangla aliases (spec §7.1). `[Must]`

## Phase B — Intake & normalization (T0-1)

- [x] **B1. Normalizers** — `src/agent/normalize.ts`: area→ha (acre/decimal/bigha-confirm/kani-ask),
...
- [x] **B2. LLM extractor + prompts** — `src/agent/prompts.ts` extractor schema (Bangla-aware),
...
- [x] **B3. Gap loop** — `src/agent/intake.ts`: `requiredFieldGaps`, ask only missing (≤3/turn),
...
- [x] **B4. Texture/fertility split** — SRDI district default → `fertilityClass` with
  `fertilitySource`, stated as an overridable assumption (spec §1.3). `[Must]`

## Phase C — Grounding tools (T0-2)

- [x] **C1. `geocodeLocation`** — Open-Meteo geocoding; ambiguity → confirm. `[Must]`
- [x] **C2. `getForecast`** — 16-day daily; derive next-7 / next-16 totals + tmean. `[Must]`
- [x] **C3. `getClimateNormals`** — archive API, average by month **in code**, label
  `historical normal (2016–2025)`. `[Must]`
- [x] **C4. `getSoilProfile`** — SRDI district fertility default lookup. `[Must]`
- [x] **C5. Failure + cache policy** — retry once, never invent, failed call visible in trace;
...
- [x] **D1. `rankCrops`** — `src/agent/ranking.ts`, weights constant in one place; subscores
...
- [x] **D2. `computeFinancials`** — `src/engines/financials.ts` pure fn: itemized costs, yield,
  revenue, net, ROI, break-even price & yield; persist both break-even fields on `SeasonPlan`
  (spec §5). `[Must]`

## Phase E — Plan generator (T0-4)

- [x] **E1. `generateSeasonPlan`** — all required stages; urea splits from FRG row (not
  hardcoded); irrigation from water_row × availability. `[Must]`
- [x] **E2. Date policy** — anchor = farmer date else window midpoint, shown as editable
  assumption. `[Must]`
- [x] **E3. Weather-note hook** — ≥40 mm rain within ~2 days of a fertilizer task in the 16-day
  window → delay note. `[Must]`

## Phase F — Orchestration & render (T0-6 / T0-8)

- [x] **F1. Orchestrator** — `src/agent/orchestrator.ts` 8-step pipeline (plan.md §3), persists to
  `SeasonPlan(+Item)`, `AgentSession`. `[Must]`
- [x] **F2. Recommendation-basis block** — built **in code** from profile + trace, injected;
  narrator writes prose around it (spec §7-orig template). `[Must]`
- [x] **F3. Routes** — `POST /api/agent/message`, `GET /api/sessions/:id/trace`; wire into
  `src/app.ts`. `[Must]`
...
- [x] **G1. `units.test.ts`** — 2 acres→0.8094 ha · 100 decimals→1 acre · maund price→per-kg. `[Must]`
- [x] **G2. `fertScaling.test.ts`** — dose kg/ha × area linear (1 / 2 / 0.5 ha). `[Must]`
- [x] **G3. `financials.test.ts`** — golden farm hand-checked; double area ⇒ cost/yield/revenue/
  profit double; per-ha figures unchanged. `[Must]`
- [x] **G4. `missingData.test.ts`** — unknown crop → no invented numbers, offers covered crops;
...
- [x] **G5. `trace.test.ts`** — golden farm end-to-end → every numeric value in final output maps
  to ≥1 `AgentToolCall`. **Rubric line — do not cut.** `[Must]`
- [x] **G6. `ranking.test.ts`** — lowering `waterAvailability` lowers `waterFit` for a high-water
